    ...
    var convertDataRequestToHTTP = function (type, resource, params) {
        ...
        switch (type) {
            case GET_LIST: {
                ...
                var query = __assign({}, fetchUtils.flattenObject(filter), { _sort: field + ':' + order, _start: (page - 1) * perPage, _limit: perPage });
                if (q)
                    query['_q'] = q;
            ...
            case GET_MANY_REFERENCE: {
                ...
                var query = __assign({}, fetchUtils.flattenObject(params.filter), (_a = {}, _a[params.target] = params.id, _a._sort = field + ':' + order, _a._start = (page - 1) * perPage, _a._limit = perPage, _a));
                url = apiUrl + "/" + resource + "?" + stringify(query);
                break;
            }
            case UPDATE:
